                    ...
                    {#each schedules as { currentstep, groupid }, i}
                        <div class="schedule-box" on:click={alertFunc(`/dash/schedules/${groupid}`)}>
                            ...
                            <div class="schedule-info-text">
                                <h3 style = "font-family: 'Raleway', sans-serif;">{actionNeeded[i] ? stepToName[currentstep] : 'Waiting for other members'}</h3>
                                <p style = "font-family: 'Raleway', sans-serif;" class="schedule-members">{groupMem[i]}</p>
                            </div>
                        </div>
                    {/each}
                </div>
